function [tableMutations_FunSeq2] = loadTableMutations_FunSeq2_context50(runAgain, tissueName, biosampleABC, tableMutations, tableSamples, sProperties)
...
if (runAgain || ~exist(saveFileData, 'file'))
    ...
    lstCols1 = {'chr', 'pos0', 'pos1', 'ref', 'alt', 'FunSeq2_gerp', 'FunSeq2_noncodingScore', 'FunSeq2_isMOTIFBR', 'FunSeq2_isMOTIFG', 'FunSeq2_motifAnalysis', 'context50bp'};

    for iProject = 1:length(lstProjects)
        projectName = lstProjects{iProject};
        t1 = tic;
%         try
            tableAnnotatedFunSeq2 = readtable([sProperties.FUNSEQ2_DIR, 'intersectedC.',projectName,'.motif.FunSeq2.context50bp.bed.txt'], 'ReadVariableNames', true); % ['data/FunSeq2/annotatedPCAWG/intersectedB.',projectName,'.motif.FunSeq2.bed.txt']
            tableAnnotatedFunSeq2.Properties.VariableNames = {'chr', 'pos0', 'pos1', 'ref', 'alt', 'FunSeq2_chr', 'FunSeq2_pos0', 'FunSeq2_pos1', 'FunSeq2_ref', 'FunSeq2_alt', 'FunSeq2_gerp', 'FunSeq2_cds', 'FunSeq2_motifAnalysis', 'FunSeq2_noncodingScore', 'context50bp'};
...
%         end
        ...
        if (~isempty(tableAnnotatedFunSeq2))
            ...
            tableAnnotatedFunSeq2_unique = tableAnnotatedFunSeq2; % Now we don't do unique (it would be better, but doesn't work on string variables and we would need to sort out FunSeq2_motifAnalysis variable. Non-unique is sufficient for this use though.
            ...
            lstCols2 = {'FunSeq2_gerp', 'FunSeq2_noncodingScore', 'FunSeq2_isMOTIFBR', 'FunSeq2_isMOTIFG', 'FunSeq2_motifAnalysis', 'context50bp'}; % 'MOTIFBR_motifName', 'MOTIFBR_motifStart', 'MOTIFBR_motifEnd', 'MOTIFBR_positionMutation', 'MOTIFBR_isMinusStrand', 
    ...
    %%
    lstCols3 = {'pos1', 'FunSeq2_isAnnotated', 'FunSeq2_isMOTIFBR', 'FunSeq2_isMOTIFG', 'FunSeq2_motifAnalysis', 'context50bp', 'FunSeq2_gerp', 'FunSeq2_noncodingScore', 'isInABCEnhancer', 'isNearTSS_250bp'};
...
%% Checking that the positions match
...
tmp1 = tableMutations_FunSeq2(tableMutations_FunSeq2.FunSeq2_isAnnotated,:);
tmp1.context8bp = cellfun(@(x) upper(x(43:59)), tmp1.context50bp, 'UniformOutput', false);
if (~isequal(tableMutations.context(tableMutations_FunSeq2.FunSeq2_isAnnotated), tmp1.context8bp)), error('Contexts do not match'); end
%%