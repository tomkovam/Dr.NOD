...
cTrinucleotides = loadTrinucleotides();
...
lstGenesFalsePositives_group2_blood = [];
for iRow = 1:length(tmp3)
    ...
    lstGenesFalsePositives_group2_blood = [lstGenesFalsePositives_group2_blood; tmp4.geneSymbol(tmp4.pE>tmp4.pE(1))];
end
lstGenesFalsePositives_group2_blood = unique(lstGenesFalsePositives_group2_blood);
lstGenesFalsePositives_group1_blood = {'ZNF876P', 'WEE1', 'AICDA', 'BCAT1', 'C12orf77', 'PPM1F', 'TOP3B', 'PRAMENP'}; % computed in a separate script
%%
% fig = createMaximisedFigure(1);
% hold on;
% % histogram(tmp.sizeEffectE(tmp.isDriver==0 & tmp.isUP & tmp.iTissue>1));
% % histogram(tmp.sizeEffectE(tmp.isDriver==1 & tmp.isUP & tmp.iTissue>1));
% histogram(tmp.sizeEffectE(tmp.literatureEvidenceOncogene==0 & tmp.isUP & tmp.iTissue>1), 0:0.1:2);
% histogram(tmp.sizeEffectE(tmp.literatureEvidenceOncogene>0 & tmp.isUP & tmp.iTissue>1), 0:0.1:2);
% isOK = tmp.isUP & tmp.iTissue>1;
% mean(tmp.literatureEvidenceOncogene(isOK)>0)
% mean(tmp.literatureEvidenceOncogene(isOK)>1)
% mean(tmp.literatureEvidenceOncogene(isOK)>2)
% isOK = tmp.isUP & tmp.iTissue>1 & tmp.sizeEffectE>0.5;
% mean(tmp.literatureEvidenceOncogene(isOK)>0)
% mean(tmp.literatureEvidenceOncogene(isOK)>1)
% mean(tmp.literatureEvidenceOncogene(isOK)>2)
%%
...
tmp.potentialFalsePositive(ismember(tmp.geneSymbol, [{'HCG15', 'CPOX', 'CLTC'}, lstGenesFalsePositives_group1_blood])) = 1; % Genes in flanking regions have also regulatory mutation count higher than expected by the background mutagenesis model.
tmp.potentialFalsePositive(ismember(tmp.geneSymbol, [{'ZFP62', 'CCNB1IP1', 'ALOXE3'}, lstGenesFalsePositives_group2_blood'])) = 2; % Genes that share regulatory driver mutations (always the gene with the highest scoreE is kept).
